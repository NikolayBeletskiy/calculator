...
def create_math_button(text, /, command=None, rely=0., relx=0., bacground="#f0f0f0"):
    if command is None:
        def command():
            global line
            line += text
    ...
    btn = tkinter.Button(frame, text=text, command=command, background=bacground)
...
create_math_button("+", rely=.8, relx=.75, bacground="blue")
create_math_button("-", rely=.6, relx=.75, bacground="blue")
create_math_button("*", rely=.4, relx=.75, bacground="blue")
create_math_button("/", rely=.2, relx=.75, bacground="blue")

create_math_button("(")
create_math_button(")", relx=.25)

create_math_button("=", command=calculate, rely=.8, relx=.5, bacground="yellow")
create_math_button("c", clear, relx=.5, bacground="red")
create_math_button("<-", backspace, relx=.75, bacground="red")
